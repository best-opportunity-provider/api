...
from database.models.trans_string.embedded import ContainedTransString, Language


FILE_PATH = 'scripts/country.csv'
DEBUG = True


def get_existing_counties() -> list[str]:
    return [str(country.name.en) for country in models.Country.objects]
...
def save_country_to_db(name_en: str, name_ru: str, phone_code: str, flag_emoji: str):
    ...
    name = ContainedTransString(en=name_en, ru=name_ru, fallback_language=Language.ENGLISH)
    country = models.Country(name=name, phone_code=phone_code, flag_emoji=flag_emoji)
    country.save()
